...
# Mostrar detalles de conglomeración
def print_clustering_details(Z):
    print("Historial de conglomeración:")
    print("Etapa | Clúster 1 | Clúster 2 | Coeficiente")
    for i, (clust1, clust2, dist, size) in enumerate(Z):
        # Los clusters en `linkage` están indexados desde 0
        clust1 = int(clust1)
        clust2 = int(clust2)
        print(f"{i+1:<5} | {clust1:<8} | {clust2:<8} | {dist:.4f}")

print_clustering_details(Z)
